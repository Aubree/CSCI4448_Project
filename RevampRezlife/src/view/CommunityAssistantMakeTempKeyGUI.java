...
public class CommunityAssistantMakeTempKeyGUI extends JFrame implements ActionListener {
	...
	private void buildGUI() {
		...
		make_temporary_key_button = new JButton("Make Temp");
	...
	private void tempKeyResponse(int choice) {
		if (choice == JOptionPane.YES_OPTION) {
			...
			if (result == true) {
				...
				choice = JOptionPane.showConfirmDialog(null, "Failed to make temporary key. Retry?");
				if (choice == JOptionPane.YES_OPTION) {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (object == make_temporary_key_button) {
			int choice = JOptionPane.showConfirmDialog(null, "Are you sure you want to make a Temporary Key?");
			tempKeyResponse(choice);
		}
